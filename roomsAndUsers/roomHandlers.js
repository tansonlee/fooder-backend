...
const handleJoinRoom = (io) => {
  return (username, roomId) => {
    ...
    // io.to(roomId).emit("user joined", currentUsers);
...
const handleChangeRoomCharacteristics =
  (io) => (userId, roomId, newCharacteristics) => {
    console.log(
      `${user} changing room ${roomId} characteristics to ${newCharacteristics.toString()}`
    );
    const room = Rooms.changeRoomCharacteristics(
      userId,
      roomId,
      newCharacteristics
    );
